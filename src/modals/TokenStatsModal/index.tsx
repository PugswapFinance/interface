...
export default function TokenStatsModal({ token }: { token: any }) {
  ...
  if (token.symbol == 'CRO') tokenInfo = { circulatingSupply: '25000000000', burnt: '0', totalSupply: '0', vaults: '0' }

  const price = formatNumberScale(priceData?.data?.[token.symbol.toLowerCase()], true, 2)

  const modalOpen = useModalOpen(token.symbol == 'SILVER' ? ApplicationModal.SOLAR_STATS : ApplicationModal.MOVR_STATS)
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex items-center justify-between space-x-3 gap-2">
                {token.address && (
                  <ExternalLink
                    href={
                      'https://cronos.crypto.org/explorer/testnet3/tokens/0x810DC08096E28C1306818A71a196F4E17d8D9cB4/'
                    }
