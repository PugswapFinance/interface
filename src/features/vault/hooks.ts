...
export function usePendingSilver(farm) {
  ...
  const result = useSingleCallResult(args ? contract : null, 'pendingSilver', args)?.result
...
export function useSolarPositions(contract?: Contract | null) {
  ...
  return useMemo(() => {
    if (!pendingSilver || !userInfo || !userLockedUntil) {
      return []
    }
    return zip(pendingSilver, userInfo, userLockedUntil)
      .map((data, i) => ({
        id: args[i][0],
  ...
  }, [args, pendingSilver, userInfo, userLockedUntil])
}
...
export function useVaultInfo(contract) {
  const SilverPerBlock = useSingleCallResult(contract ? contract : null, 'SilverPerBlock', undefined, NEVER_RELOAD)
    ?.result?.[0]

  const totalAllocPoint = useSingleCallResult(contract ? contract : null, 'totalAllocPoint', undefined, NEVER_RELOAD)
    ?.result?.[0]

  return useMemo(() => ({ SilverPerBlock, totalAllocPoint }), [SilverPerBlock, totalAllocPoint])
}
