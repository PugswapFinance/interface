...
const builders = {
  ...
  cronosTestnet: (chainName: string, data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
    const prefix = 'https://cronos.crypto.org/explorer/testnet3/'
    switch (type) {
      case 'transaction':
        return `${prefix}/tx/${data}`
      case 'address':
...
const chains: ChainObject = {
  ...
  [ChainId.CRONOS_TESTNET]: {
    chainName: '',
    builder: builders.cronosTestnet,
  },
  [ChainId.CRONOS]: {
    chainName: '',
